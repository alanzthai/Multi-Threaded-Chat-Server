...
Test(blackbox_suite, 02_connect_alice, .init = init, .fini = fini, .timeout = 10) {
    int ret = system("(echo login alice ; sleep 5 ; echo send bob hello; sleep 1) | util/client -p 9999");
    cr_assert_eq(ret, 0, "expected %d, was %d\n", 0, ret);
}

Test(blackbox_suite, 03_connect_bob, .init = init, .fini = fini, .timeout = 10) {
    int ret = system("(echo login bob ; sleep 5 ; echo send alice hello; sleep 1) | util/client -p 9999");
    cr_assert_eq(ret, 0, "expected %d, was %d\n", 0, ret);
}

Test(blackbox_suite, 04_connect_carol, .init = init, .fini = fini, .timeout = 10) {
    int ret = system("(echo login carol ; sleep 5 ; echo send carol hello; sleep 1) | util/client -p 9999");
    cr_assert_eq(ret, 0, "expected %d, was %d\n", 0, ret);
}

Test(blackbox_suite, 03_connect_alan, .init = init, .fini = fini, .timeout = 10) {
    int ret = system("(echo login alan ; sleep 5 ; echo send alice hello; sleep 1) | util/client -p 9999");
    cr_assert_eq(ret, 0, "expected %d, was %d\n", 0, ret);
}

Test(blackbox_suite, 04_connect_tom, .init = init, .fini = fini, .timeout = 10) {
    int ret = system("(echo login tom ; sleep 5 ; echo send carol hello; sleep 1) | util/client -p 9999");
    cr_assert_eq(ret, 0, "expected %d, was %d\n", 0, ret);
}
